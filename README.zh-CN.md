...
- **窗口标签（快捷入口）**：在缩略图界面选中某窗口后**连续按两下 `P`**，即可给该窗口
  添加一个标签，缩略图右上角显示琥珀色数字角标（纯标识，不影响排序/布局）。
  标签按添加顺序编号 **1/2/3，最多 3 个**；按下 `⌘⇧1` / `⌘⇧2` / `⌘⇧3`（左右
  Shift 均可）**随时直接快速切换到对应标签窗口**。标签窗口关闭后其它标签不补位，
  新添加的标签自动占用最小编号（如标签 1 关闭后，新标签即为 1）；再次对已标签
  窗口双击 `P` 可移除标签；达到上限时有轻微提示。
